...
// +-------------------------------------------------+
// $Id: c39object.php,v 1.4.12.2 2025/03/27 13:29:26 dbellamy Exp $
    ...
    public function DrawObject($xres)
    {
        ...
        if ($this->mStyle & BCS_DRAW_TEXT) {
            if ($this->mStyle & BCS_STRETCH_TEXT) {
                for ($i = 0; $i < $len; $i ++) {
                    $this->DrawChar($this->mFont, $sPos + ($size / ($len + 2)) * ($i + 1.5) - $this->GetFontWidth($this->mFont) / 2, $ysize + BCD_DEFAULT_MAR_Y1 + BCD_DEFAULT_TEXT_OFFSET, $this->mValue[$i]);
                }
            } else { /* Center */
                $text_width = $this->GetFontWidth($this->mFont) * strlen($this->mValue);
                $this->DrawText($this->mFont, $sPos + (($size - $text_width) / 2), $ysize + BCD_DEFAULT_MAR_Y1 + BCD_DEFAULT_TEXT_OFFSET, $this->mValue);
            }
        }
