...
// +-------------------------------------------------+
// $Id: custom_fields_query_auth.class.php,v 1.2.4.3 2025/02/20 15:18:36 dgoron Exp $
        ...
        for ($i=0; $i<$n; $i++) {
            ...
            if (($i==0)) {
                if ($options['MULTIPLE'][0]['value']=="yes") {
                    ...
                    $browse_link = $base_path."/select.php?what=$what&caller=$caller&p1=".$field["NAME"]."_$i&p2=f_".$field["NAME"]."_$i&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&dyn=$val_dyn&perso_name=".$field['NAME']
                    ."&max_field=n_".$field["NAME"]."&field_id=".$field["NAME"]."_&field_name_id=f_".$field["NAME"]."_&add_field=add_".$field["NAME"]."&id_thes_unique=".urlencode($id_thes_unique).$element;
                    $ret .= static::get_button_browse($field, $browse_link);
                    $ret.= static::get_button_add($field, 'add_'.$field["NAME"].'();');
                }else {
                    $browse_link = $base_path."/select.php?what=$what&caller=$caller&p1=".$field["NAME"]."_$i&p2=f_".$field["NAME"]."_$i&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&dyn=$val_dyn&perso_name=".$field['NAME']
                    ."&max_field=n_".$field["NAME"]."&field_id=".$field["NAME"]."_0&field_name_id=f_".$field["NAME"]."_0&add_field=add_".$field["NAME"]."&id_thes_unique=".urlencode($id_thes_unique).$element;
                    $ret .= static::get_button_browse($field, $browse_link);
                }
            }
