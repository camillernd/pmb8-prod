...
// +-------------------------------------------------+
// $Id: cms_editorial_data.class.php,v 1.21.2.2 2025/01/08 14:32:05 qvarin Exp $
...
class cms_editorial_data extends cms_root
{
    ...
    public function __get($attribute)
    {
        ...
        if (method_exists($this, "get_".$attribute)) {
            return call_user_func_array([$this, "get_".$attribute], []);
        } elseif (isset($this->{$attribute})) {
            return $this->{$attribute};
        } elseif (method_exists($this, $attribute)) {
            return call_user_func_array([$this, $attribute], []);
        } elseif (method_exists($this, "is_".$attribute)) {
            return call_user_func_array([$this, "is_".$attribute], []);
            ...
            foreach ($this->fields_type as $key => $value) {
                switch ($key) {
                    ...
                    case "article_title_".$lang_tmp:
                        if ($value) {
                            $this->title = $value["values"][0]["format_value"];
                        }
                        break;
                    case "section_resume_".$lang_tmp:
                    case "article_resume_".$lang_tmp:
                        if ($value) {
                            $this->resume = $value["values"][0]["format_value"];
                        }
                        break;
                    case "article_content_".$lang_tmp:
                    case "article_contenu_".$lang_tmp:
                        if ($value) {
                            $this->contenu = $value["values"][0]["format_value"];
                        }
                        break;
                    default:
                        if (strpos($key, $lang_tmp) !== false) {
                            if ($value) {
                                $this->fields_type[str_replace("_".$lang_tmp, "", $key)] = $value;
                            }
                        }
                        break;
                }
