...
// +-------------------------------------------------+
// $Id: cms_module_search_view_search.class.php,v 1.78.2.4 2025/01/21 16:35:21 dgoron Exp $
...
class cms_module_search_view_search extends cms_module_common_view{
	...
	public function render($datas){
		...
		//juste une searchbox...
		if(is_countable($datas) && count($datas) == 1){
		...
		if (is_countable($this->parameters['others_links']) && count($this->parameters['others_links'])) {
		    if ($opac_rgaa_active) {
		        foreach ($this->parameters['others_links'] as $key => $other_link) {
		            $html .= "
						<a class='search_link search_other_link' id='search_other_link_" . $key . "' href='" . $other_link['url'] . "' " . (isset($other_link['linktarget']) && $other_link['linktarget'] ? "target='_blank'" : "") . " ".(!empty($other_link['title']) ? "title='" . $this->format_text($other_link['title']) . "'" : "").">" . $this->format_text($other_link['label']) . "</a>";
		        }
		    } else {
		        foreach ($this->parameters['others_links'] as $key => $other_link) {
		            $html .= "
						<p class='search_other_link' id='search_other_link_" . $key . "'><a href='" . $other_link['url'] . "' " . (isset($other_link['linktarget']) && $other_link['linktarget'] ? "target='_blank'" : "") . " ".(!empty($other_link['title']) ? "title='" . $this->format_text($other_link['title']) . "'" : "").">" . $this->format_text($other_link['label']) . "</a></p>";
		        }
		    }
		}
		if ($opac_rgaa_active) {
