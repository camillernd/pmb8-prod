...
// +-------------------------------------------------+
// $Id: proxy.js,v 1.1.4.2 2025/04/17 13:26:48 qvarin Exp $
...
nedap = new Nedap(url_serveur_rfid);
...
function init_rfid_read_cb(empr_client, expl_client) {
	...
	if (nedap.isReturnExplPage()) {
		// On a deriver la function pour nedap
...
function read_cb() {
	...
	var multiple = false;
	if (nedap.isReturnExplPage() || nedap.isRFIDReadPage() || nedap.isEditExplPage()) {
	...
	nedap.getItems(multiple).then(result_read_cb);
}
...
function mode1_read_cb(fromInterval) {
	...
	nedap.getItems(true).then(mode1_result_read_cb);
}
...
async function read_uid(f_ack) {
	...
	const result = await nedap.getItems(true);
...
function init_rfid_detect(ack_detect) {
	...
	nedap.getItems(true).then(result_detect).catch(afficheErreur);
}
...
async function rfid_erase_suite(resultItems) {
	...
	for (let i = 0; i < resultItems.length; i++) {
		...
		success &= await nedap.clearTag(item.tagId);
	}
...
function init_rfid_write_etiquette(cb, nbtags, ack_write) {
	...
	nedap.writeExpl(cb, nbtags).then(result_write);
}
...
function init_rfid_write_empr(cb, ack_write) {
	...
	nedap.writeEmpr(cb).then(result_write_empr);
}
...
async function init_rfid_antivol(cb, level, ack_antivol) {
	...
	if (!get_uid_from_cb[cb]) {
		result = await nedap.setTagSecurity(afi == 'true', cb);
		...
		result = await nedap.setTagSecurity(afi == 'true');
	}
...
function init_rfid_antivol_all(level, ack_antivol) {
	...
	nedap.getItems(true).then(result_rfid_antivol_1);
}
...
async function result_rfid_antivol_1(result) {
	...
	for (let index in items) {
		success &= await nedap.setTagSecurity(afi == 'true', items[index].cb);
	}
